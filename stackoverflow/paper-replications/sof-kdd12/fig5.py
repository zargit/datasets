...
votes = pd.read_sql_query("select postid, votetypeid from votes where creationdate between \'2017-07-01\' and \'2017-12-31\'", mydb);
...
posts = pd.read_sql_query("select id, answercount, commentcount, unix_timestamp(creationdate) as creationdate, owneruserid, parentid, acceptedanswerid, posttypeid from posts where creationdate between \'2017-07-01\' and \'2017-12-31\'", mydb)
...
answers[['pos_votes', 'neg_votes', 'accepted']] = answers[['pos_votes', 'neg_votes', 'accepted']].fillna(0)

answers['reputation_won'] = (answers['pos_votes']*10) + (answers['neg_votes']*(-2)) + (answers['accepted']*15)
...
q2 = qa[qa['answercount_q']==2].groupby('timerank')['reputation_won'].mean()
q3 = qa[qa['answercount_q']==3].groupby('timerank')['reputation_won'].mean()
q4 = qa[qa['answercount_q']==4].groupby('timerank')['reputation_won'].mean()
q5 = qa[qa['answercount_q']==5].groupby('timerank')['reputation_won'].mean()

q2 = pd.DataFrame({'x':range(1,3), '2':q2})
q3 = pd.DataFrame({'x':range(1,4), '3':q3})
q4 = pd.DataFrame({'x':range(1,5), '4':q4})
q5 = pd.DataFrame({'x':range(1,6), '5':q5})
...
plt.ylabel("Average reputation won per answer", fontsize=18)
plt.legend()
plt.show()
