...
posts = pd.read_sql_query("select id, answercount, commentcount, unix_timestamp(creationdate) as creationdate, owneruserid, favoritecount, parentid, acceptedanswerid, posttypeid, score, viewcount, postlength from posts where creationdate between \'2009-12-01\' and \'2009-12-31\'", mydb)
...
for i in t:
	x = qa[qa['delay'] <= i*3600][['id_q', 'score_a', 'reputation_a', 'favoritecount_q', 'viewcount_q', 'answercount_q']]
	max_score_row = x.sort_values('score_a', ascending=False).drop_duplicates(['id_q'])
	max_score_row.columns = ['id', 'max_score', 'max_score_answerer_reputation', 'favoritecount', 'pageview', 'answercount']
	max_reputation_answerer = x.sort_values('reputation_a', ascending=False).drop_duplicates(['id_q'])[['id_q', 'reputation_a']]
	...
	data.to_csv('Sb_%dhour.csv'%(i), index=False)